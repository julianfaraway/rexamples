...
We load the `lmerTest` package which provides inferential methods on
top of `lme4` (which is loaded in addition)
...
We find no significant difference in the treatments. We
can use the Kenward-Roger method with:
...
model such as this, there is no need for adjustment or doubt about
the null F-distribution.


Testing the random effects is more challenging. 
We test the hypothesis $H_0: \sigma^2_v = 0$.
The `lmerTest` package
implements a "random effects anova":
```{r}
ranova(mmod)
```
...
We get lots of warning messages about a boundary fit. The proportion
of bootstrapped test statistics that are equal to zero is:
```{r}
mean(lrstatf == 0)
```
...
ls_means(mmod)
```
In this case, the result is the same as before. A different degrees of freedom
method has been used but it makes no difference in this example.

Pairwise comparisons are also possible:
```{r}
difflsmeans(mmod)
```
The estimates and standard errors are the same as before but the confidence
intervals differ because `emmeans` is making a multiple comparisons adjustment
while `lmerTest` is not.
