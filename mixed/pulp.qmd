...
y_{ij} = \mu + \alpha_i + \epsilon_{ij} \qquad i=1,\dots ,a
  \qquad j=1,\dots ,n_i,
$$
where the $\alpha_i$ and $\epsilon_{ij}$s are normal
...
decide between them and so a qualitative comparison is entirely reasonable.
...
approximations but feel comfortable with accepting. No one wants to open a can of worms. But eventually the
...
problems that I have only the vaguest ideas about solving. Of course, it would be 
more practical to ask them to use a central server where everything is installed 
but this results in other problems regarding access and usage. After several
decades of fiddling with computers to get software to work, I (mostly) possess
the patience and experience to get these things to work. That's far from true
